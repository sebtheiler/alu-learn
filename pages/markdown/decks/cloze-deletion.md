Cloze deletion is a powerful technique that lets you obscure one or multiple phrases from a flashcard.  When studying, the specified phrase will be hidden from you and when you reveal the answer it will be shown.

To create cloze flashcards, select the "Cloze" option as the flashcard type when creating the flashcard.  Next, follow a simple syntax: "words words words {{c1::hidden word}} words words words"
...
The above example would actually create two different flashcards - one where Kaleida is hidden and another where $40 million is hidden.  Note that the first cloze uses the number "1", while the second cloze uses the number "2".
