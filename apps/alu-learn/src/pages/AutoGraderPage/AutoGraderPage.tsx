...
import { RubricI, defaultRubric } from "./rubricPresets";
...
export default function AutoGraderPage({
  ...
  const [rubric, setRubric] = useState<RubricI>(defaultRubric);
  ...
  const autoGradeEssay = async () => {
    ...
    if (prompt.length < MIN_PROMPT_LENGTH) {
      setPromptError(
        `Prompt must be at least ${MIN_PROMPT_LENGTH} characters long. It is currently ${prompt.length} characters long.`
      );
      isError = true;
    }
    if (essay.length < MIN_ESSAY_LENGTH) {
      setEssayError(
        `Essay must be at least ${MIN_ESSAY_LENGTH} characters long. It is currently ${essay.length} characters long.`
      );
      isError = true;
    }
    if (isError) return;
  ...
  return (
    <>
      <SEO
        ...
        path="auto-grader"
        description="Get instant feedback on your essays with Alu Learn's automatic grader. Alu grades your work on your rubric and provides detailed suggestions for improvement. Easily improve your writing skills and boost your grades."
      />
      <div className="mt-28">
        <div className="grid grid-cols-12">
          <div className="col-span-12 md:col-span-3 px-2">
            <Ad adType="AUTO_GRADER_SIDE_1" />
          </div>
          <div className="col-span-12 md:col-span-6 px-2">
            <h1 className="font-bold text-center text-4xl">Auto-Grader</h1>
            <p className="text-center text-gray-700 max-w-lg mx-auto text-lg mb-1">
              Want feedback on your essay? Auto-grader can preview how well it
              will do on a rubric and suggest points to improve on
            </p>
            <p className="text-center text-xs text-gray-500 max-w-md mx-auto mb-3">
              Auto-grader&apos;s responses to your essay are not guaranteed to
              align with your teacher&apos;s perspective, but they still offer
              valuable feedback
            </p>
            ...
            />
            {promptError && (
              <p className="text-center text-red-600">{promptError}</p>
            )}
            ...
            {essayError && (
              <p className="text-center text-red-600">{essayError}</p>
            )}
            {wordCount > maxWordCount && (
              <p className="text-center text-red-600 font-bold">
                Your essay is too long! Please shorten it to {maxWordCount}{" "}
                words or less (currently {wordCount})
              </p>
            )}
            {exceededQuota && (
              <div>
                <p className="text-red-600 my-2 text-center font-bold">
                  You have exceeded your monthly quota of automatic essay
                  grades.{" "}
                  {isPro ? (
                    <>
                      Contact{" "}
                    ...
                    <>
                      Upgrade to pro to generate up to continue using this
                      feature
                    </>
                  )}
                </p>
            ...
            {grades && (
              <div>
                ...
                {feedbackLoading && (
                  <p className="text-red-600 my-2 text-center">
                    Warning: This may take up to several minutes depending on
                    the length of your essay. Do <strong>not</strong> refresh
                    the page while you wait.
                  </p>
                )}
              </div>
            )}
          ...
          <div className="col-span-12 md:col-span-3 px-2">
            <Ad adType="AUTO_GRADER_SIDE_2" />
          </div>
        </div>
        <div className="px-2">
          <Ad adType="AUTO_GRADER_BOTTOM" />
        </div>
      </div>
    </>
  );
