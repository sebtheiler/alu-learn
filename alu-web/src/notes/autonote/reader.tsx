...
interface AutoReaderProps {
  selectedPar: number;
  setSelectedPar(selectedPar: number): void;
  updateProgressBar(selectedPar: number): void;
  finished: boolean;
  text: string[]; /**Array of string that represents HTML */
  showCompiledNotes?: boolean;
  setShowCompiledNotes?(showCompiledNotes: boolean): void;
  inputType: 'text' | 'video';
  compiledNotesButton?: boolean;
  children: ReactNode | ReactNodeArray;
}
export function AutoReader(props: AutoReaderProps) {
  const {selectedPar, setSelectedPar, updateProgressBar, finished, text, showCompiledNotes, setShowCompiledNotes, inputType} = props;
  const compiledNotesButton = props.compiledNotesButton ?? true;
  ...
  return (<div className='container mt-5'>
    ...
    <ButtonGroup className='mt-1 float-right'>
      ...
      {compiledNotesButton && setShowCompiledNotes && <Button
        variant='secondary'
