...
const checkHeadingMatch = (
  element: Text,
  targetText?: string,
  targetHeadingSize?: number,
) => {
  return (
    (element as any).type.startsWith('heading-') && // check that it's a heading
    (!targetHeadingSize || wordToNum[(element as any).type.substring(8)] === targetHeadingSize) && // check the heading size is correct
    (!targetText || (element as any).children[0].text === targetText) // check the text is correct
  );
}
