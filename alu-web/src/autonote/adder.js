...
const sectionString = 'Second Header 1 >  Second Header 2';
...
const insertElement = (data, headingSize, startingIndex=0) => {
  console.log(data)
  var elementMatch;
  console.log('searching', headingSize, startingIndex, data.length)
  for (const [index, element] of data.slice(startingIndex).entries()) {
    if (elementMatch) {
      ...
      if (checkHeadingMatch(element) || index === data.length - 1) {
        ...
        if (newHeadingSize <= headingSize || parsedSection.length === headingSize) {
          console.log('appending at', index)
