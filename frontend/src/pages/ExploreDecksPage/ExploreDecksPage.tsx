  ...
  return (
    <div className='container px-48 mx-auto mt-28'>
      <div>
        ...
        <p className='text-lg'>Find top decks created by others to help you study</p>
      </div>
      <hr className='my-3' />
      <div>
        {sharedDecks.map(sharedDeck =>
          <div className='bg-alu-light-gray border-4 border-alu-mid-gray px-6 py-4 rounded-xl' key={sharedDeck.id}>
            <div>
              <p className='float-right -translate-y-1'>
                Created by {sharedDeck.owners.map((owner, i) => <Fragment key={i}>
                  <DisplayProfileInline profile={owner} />
                  {i !== sharedDeck.owners.length - 1 && ', '}
                </Fragment>)}
                <br />
                {sharedDeck.numCopies} {sharedDeck.numCopies === 1 ? 'copy' : 'copies'}
              </p>
              <h1 className='text-4xl font-bold my-2 text-blue-500 hover:text-blue-600 hover:underline'>
                <Link to={`/community/decks/${sharedDeck.id}/`}>
                  {sharedDeck.title}
                </Link>
              </h1>
              <hr className='my-3' />
