...
import { faGraduationCap, faNewspaper, faPersonChalkboard, faPlus, faUserGroup, faWindowRestore } from '@fortawesome/free-solid-svg-icons';
...
export default function WelcomeSurveyPage() {
  ...
  const slides = useMemo(() => {
    ...
    if (answers.userType === 'STUDENT') {
      ...
      slides = slides.concat([
        (<>
          ...
          ]} onClick={handleNext('joinReason')} shuffle numCols={4} />
        </>),
        (<>
          ...
          ]} onClick={handleNext('targetFlashcards')} numCols={4} />
        </>),
        (<>
      ...
      slides.push(<>
        <h4>Why did you join Alu?</h4>
        <ChoiceSelect choices={[
          { value: 'STUDENTS', display: 'I want to use it with my students' },
          { value: 'CONCEPT', display: 'I think it\'s an interesting concept' },
        ]} onClick={handleNext('joinReason')} shuffle includeOther />
      </>);
    }
    ...
    slides.push((<>
      ...
      <ChoiceSelect choices={[
        { value: 'CREATE_OWN', display: 'Create My Own', icon: faPlus, iconColor: 'navy' },
        { value: 'COPY_EXISTING', display: 'Copy Existing Deck', icon: faWindowRestore, iconColor: 'peru' },
