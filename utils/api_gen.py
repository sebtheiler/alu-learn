...
def create_object_view(
    ...
    `editable_attr_types`: Dictionary mapping the model's editable attributes to their types
        ({'title': str, 'num': int})
    ...
    def view(request: WSGIRequest):
        if resp := assert_request_data_type(request, {'create_values': dict}):
            return resp
        if msg := assert_dict_data_type(request.data['create_values'], editable_attr_types):
